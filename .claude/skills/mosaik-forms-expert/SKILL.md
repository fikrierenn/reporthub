...
**Gap 2 Submission admin:** hiç kod yok. Data: `db.Set<FormSubmission>().Include(Values).Include(Files)` filter FormDefinitionId+firma (ownership `FormDefinitionService.cs:22-26` deseni). Export: KVKK `VerbisExporter.cs` referans (ClosedXML XLWorkbook + **Safe() formül-injection guard** + AdjustToContents). **csproj'a `ClosedXML` PackageReference ekle** (yok). FieldKey denorm → pivot join'siz (Label için FormField join). DownloadFile(:155) attachment için reuse.
