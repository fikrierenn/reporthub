...
namespace ReportPanel.Services.Rendering
{
    ...
    // Bu dosya: client-side init scripts (510 sat) — switchTab, modal, fmtNum, aggVal, fmtKpi,
    // KPI variant dispatch, chart init, table init.
    // DashboardShellRenderer.cs: HTML iskelet + tabs + modal + grid + EndHtml (~110 sat).
...
function fmtKpi(v, fmt) {
  ...
  return fmtNum(n);
            ...
            // KPI init — ADR-008 F-5: 4 variant dispatch (basic/delta/sparkline/progress)
            sb.AppendLine(@"
...
document.querySelectorAll('[data-chart]').forEach(function(el) {
  ...
  new Chart(el, { type: chartType, data: { labels: labels, datasets: datasets }, options: options });
            ...
            // Table init — ADR-008 F-6: tableOptions + kolon format + conditional format.
            sb.AppendLine(@"
