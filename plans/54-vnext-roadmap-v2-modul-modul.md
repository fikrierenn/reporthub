...
| ~~M6~~ | ~~KVKK backbone~~ (Plan 40) | M2 | ✅ **KAPANDI 2026-07-01** commit `2fd505b` (ADR-027) | Ayrı csproj `Mosaik.Modules.Kvkk` (ADR-015). 7/7 faz kapandı: Faz 0 (backbone+EntityRelations çift-yazma), Faz 1 (xlsx import 361 süreç), Faz 2a/2b (CRUD UI), Faz 4 (reverse search), Faz 5 (AI Integrity 7/8 pattern saf-C#, Hangfire günlük, Presidio ertelendi), Faz 3 (SOP↔Process cross-modül bağ, ADR-002 izolasyonlu — junction sahipliği tek modülde, cross-area HTTP), Faz 6 (VERBİS export), Faz 7 (Risk Dashboard, EF-native Chart.js — Reports motoru mixed-access riski nedeniyle reuse edilmedi). 646 test. Full-scan'lerde 2 CRITICAL + 3 HIGH gerçek bug bulundu/kapatıldı (cross-firma sızıntı, EF çeviri hatası, sessiz veri kaybı, XSS encoder). **Sonraki: M7.** |
