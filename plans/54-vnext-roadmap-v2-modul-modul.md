...
| **M6** | **KVKK backbone** (Plan 40) | M2 | 🔄 **DEVAM — Faz 0 ✅** 2026-06-30 commit `eac6a59` | Ayrı csproj `Mosaik.Modules.Kvkk` (ADR-015). **Faz 0 KAPANDI:** KvkkProcess+DataElement+ProcessDataLink+CrossBorderTransfer + 8 REF lookup + 60 DataElement seed + EntityRelations çift-yazma (atomik) + reverse-lookup çekirdeği. Denetim kapısı tam (build+566test+4 denetim+boot/reverse smoke). **Faz 1 ✅ KAPANDI** (commit `9dad0da`): XlsxImporter 361 süreç idempotent + audit + CLI; gerçek import doğrulandı (4025 bağ, 96 yurtdışı, risk dağılımı). **Kalan fazlar:** 2 CRUD UI+sidebar, 3 SOP entegrasyon, 4 global reverse search, 5 AI integrity (Presidio), 6 VERBİS export, 7 risk dashboard. VerbisRegistration/Pattern 9 → Faz 5-6. |
