...
| ~~M3~~ | ~~Cross-module Search~~ | — | ✅ **KAPANDI 2026-06-29** commit `42fd98c` | LEAN: tek `ISearchProvider` + explicit DI + LIKE (FTS/vector YOK) + report/contract/obligation/circular provider + `/Search` + topbar arama. SOP deferred (çok-katmanlı access). Denetim kapısı tam (build+532test+4 denetim TÜMÜ temiz, security 0 leak) + authed smoke (/Search 200 "Arama"). **Sonraki: M4.** |
| ~~M4~~ | ~~Dashboard→Alert~~ (EscalationRule) | M1 | ✅ **KAPANDI 2026-06-29** commit `9961b04` | LEAN: `EscalationRule` (ReportId FK + ResultSet/Column/Aggregation/Operator/Threshold + NotifyUserIds CSV) + `EscalationSweeperJob` (Hangfire saatlik, **GLOBAL eval** user-data-filter YOK) + `INotificationService` günlük-dedup + admin CRUD (`/Escalation`) + sidebar. Denetim kapısı tam (build + **549 test** + 4 denetim TÜMÜ temiz/0 security; silent-failure F1 bildirim-sonrası state-write + F3 tr-TR parse + F4 recovery-guard fix; code-review RunNow audit + Save NewValuesJson). Smoke: /Escalation 302→Login auth + recurring-job Hangfire'da kayıtlı. NotifyUserIds CSV lean (alıcı-raporlama gerekirse `EscalationRuleRecipient` junction'a yükselt — footprint-ladder). Authed görsel = kullanıcı 5197. **Sonraki: M5.** |
| ~~M5~~ | ~~Comment/Mention~~ (Plan 35) | — | ✅ **KAPANDI 2026-06-30** commit `004fd7f` | `Mosaik.Core.Comments.Comment` polymorphic (EntityType+EntityId, FirmaId denormalize) + `ICommentService` + sanitize-on-write + `@mention` **full autocomplete** (`/Comments/Users` + comments.js) + fan-out dedup. `CommentThread` ViewComponent (modül Details app-wide çağırır, ADR-002 izolasyon). **+ NotificationDigestJob** (M1'den fold, günlük). Migration 78 + `commentEntityType` lookup. Entegre: **Contract + SOP** Details; **Circular deferred** (FirmaId yok — şirket-geneli, M2/M3 deferred pattern). Denetim kapısı tam (build + **556 test** + 4 paralel denetim). Güvenlik **H-1 fix**: write-path firma SUNUCU-TARAFI çözülür (client firmaId'ye güvenilmez). Smoke: route auth + 2 recurring job kayıtlı. **Backlog:** edit/delete yorum, mention tıklanır render, Circular firma-geneli yorum. **Sonraki: M6 KVKK backbone.** |
| **M6** | **KVKK backbone** (Plan 40) | M2 | 🔄 **DEVAM — Faz 0 ✅** 2026-06-30 commit `eac6a59` | Ayrı csproj `Mosaik.Modules.Kvkk` (ADR-015). **Faz 0 KAPANDI:** KvkkProcess+DataElement+ProcessDataLink+CrossBorderTransfer + 8 REF lookup + 60 DataElement seed + EntityRelations çift-yazma (atomik) + reverse-lookup çekirdeği. Denetim kapısı tam (build+566test+4 denetim+boot/reverse smoke). **Faz 1 ✅** (`9dad0da`): XlsxImporter 361 süreç idempotent + audit + CLI. **Faz 2a ✅** (`f44de5f`): read UI — ProcessController Index(filtre+arama)+Details(6-aspect tab) + sidebar (kvkk modülü görünür, Süreçler grubu) + KvkkLabels; 361 süreç gezilebilir, multi-tenant doğrulandı. **Faz 2b ✅** (`df4a38a`): write CRUD + ReviewStatus + audit. **Faz 4 ✅** (`05cb977`): reverse search "bu veri nerede işleniyor?" — DataElement→aktif süreç + uyumluluk skoru, modül-içi sayfa (advisor: M3 topbar'a 2. kutu yok), junction kaynak. **Faz 6 ✅** (`5bd5907`): VERBİS export (denetim-hazır Excel) + retention/disposal serbest-metin yakalama (import bloker fix, 361/361) + formula-injection guard. **Kalan:** 5 AI integrity (saf-C# 8 pattern, Presidio ertelendi), 7 risk dashboard, 3 SOP entegrasyon (SOP verisi olgunlaşınca). |
