...
**Tarih:** 2026-04-27 (revize: 2026-04-29 — alt-2W wizard fazı eklendi)
...
### F-7 alt-commit 2W — Wizard 3-step (~4h, 29 Nisan 2026 revizyon)

**Bağlam:** alt-2 (Razor split-pane + builder topbar) tamamlandı (`4ada9e6`). Ama EditReport/CreateReport hâlâ tek-sayfa: rapor metadata (Genel/Teknik/Parametreler) + Dashboard builder yan yana scroll. Mockup `app-shell-builder-v1.html` Dashboard step'inde 3-pane builder full-width gösteriyor. Çözüm: wizard wrapper.

**Karar (kullanıcı 29 Nisan):** "page şeklinde altta veya üstte olabilir wizart gibi de olabilir ama ileri geri gezilecek şekilde" → 3 step wizard, üst step indicator, alt sticky Geri/İleri/Kaydet.

**Step ayrımı:**
- **Step 1 — Genel:** Rapor Adı, Veri Kaynağı, Açıklama, Kategoriler, Aktif checkbox
- **Step 2 — Teknik:** ProcName + SP Önizle, İzinli Roller, ParamSchemaJson, Parametre Oluşturucu
- **Step 3 — Tasarım:** mockup 3-pane builder full-width (ADR-009: ayrı "dashboard" kavramı yok, rapor = tasarım)

**Davranış:**
- Tek `<form>`, tüm step input'ları POST'a dahil (x-show toggle ile DOM'da kalır)
- Topbar Kaydet her step'ten erişilebilir; sticky footer son step'te primary `Kaydet` gösterir
- Step indicator pip'ler tıklanabilir (random-access, validation engelleme yok — kullanıcı serbest)
- URL hash sync (`#step-1/2/3`) — F5'te aynı step'e döner
- Önizle butonu (topbar) sadece Step 3'te aktif (F-9'da kullanılacak placeholder)

70.5. [ ] `wizard.js` yeni dosya — Alpine `$store.wizard = { step, max:3, next(), prev(), goto() }` + URL hash sync
70.6. [ ] `dashboard-builder.css` wizard step CSS — `.wizard-steps` (üstte pip), `.wizard-step` (active/done state), `.wizard-pane` (step content), `.wizard-foot` (sticky alt)
70.7. [ ] `EditReport.cshtml` rewrite — wizard wrapper, hero kalkar, 3 step `<section x-show>`, sticky footer
70.8. [ ] `CreateReport.cshtml` aynı pattern
70.9. [ ] **Smoke:** `/Admin/EditReport/13` step nav OK, F5 sonrası hash'e dönüş, Kaydet form-wide submit
70.10. [ ] Commit: `feat(m-11 f-7): EditReport/CreateReport 3-step wizard wrapper (plan: 02)`

### F-7 alt-2V — V2 Builder Fork (29 Nisan 2026, kullanıcı pivotu)

**Bağlam**: alt-2W wizard wrap kullanıcı tarafından "değiştirmeye uğraşma, farklı isimle oluştur" denip pivot edildi. V1 sayfaları (`EditReport`, `CreateReport`, `dashboard-builder.css`, `dashboard-builder/*.js`, `wizard.js`) el değmemiş kalır. Paralel V2 sayfaları mockup `app-shell-builder-v1.html` birebir port edildi.

**Temel ilke (29 Nisan)**: **Sistem anlamlandırmaz**. Kolon adları, result set başlıkları, sektör-özel etiketler hardcoded heuristic ile tahmin edilmez. Tüm anlamlı isimlendirme admin tarafından **rapor-bazlı** olarak Drawer Veri tab'ında atanır ve `DashboardConfigJson.resultContract`'a kaydedilir → SQL'de (ReportCatalog tablosunda) kalıcı.

**Tamamlananlar**:
- `AdminController.EditReportV2(int id)` + `CreateReportV2()` actions
- `Views/Admin/EditReportV2.cshtml` + `CreateReportV2.cshtml` + `_ReportFormBuilderTopActionsV2.cshtml` partial
- `wwwroot/assets/css/builder-v2.css` — mockup birebir + responsive breakpoint (1380/1200/1024) + `.col-card`, `.rs-group`, `.bound-chip`, `.selected-bound`, `.col-search`, `.advanced-toggle`
- `wwwroot/assets/js/builder-v2/builder.js` — Alpine `x-data="builderV2"` + Gridstack imperative + drag-drop palette→canvas + drawer 3-tab + mode toggle preview render
- `ReportsController.RunJsonV2(reportId, paramsJson?)` — V1 Run path'inin JSON wrapper'ı, `_filterInjector.InjectAsync` user yetki dahil, sadece ParamSchema field'ları geçer (SP kendi default'unu kullanır)
- `GuessResultSetTitle` server-side helper — sadece "Veri Seti N" default (sektör-özel tahmin **yok**)
- `resultSetTitle` client-side — config.resultContract öncelikli, server `rs.name` fallback
- Bug fix: `.widget { position: relative }` Gridstack `.grid-stack-item { position: absolute }` override ediyordu → kaldırıldı; widget'lar artık 12-col grid'de doğru yerleşir
- Bug fix: `[object Object]` pills → kolon string normalize fetchSpPreview sonrası
- Helper'lar (builder.js): `bindColumn(rsIdx, col)`, `unbindColumn`, `setContractName`, `columnKind`, `columnSample`, `autoAggregation`, `iconChoices` (28 FA ikon)
- Drawer Veri tab rewrite (EditReportV2): `selected-bound` banner, kolon arama, `rs-group`'lar (heuristic-suzulmus + admin override edilebilir başlık), `col-card`'lar (ad + tip rozeti + örnek değer), İleri ayar collapsible (Aggregation override)
- Drawer Görünüm tab rewrite (EditReportV2): renk swatch + 28 FA icon-grid + Kaldır linki

**Yapılacak (sıralı)**:
1. ~~CreateReportV2 view senkronu~~ ✅ tamamlandı
2. ~~Palette Result Contract inline edit~~ ✅ tamamlandı (her iki view + Veri tab grupları)
3. ~~Build + smoke (login + V2 sayfa + bind/unbind/icon test)~~ ✅ tamamlandı (KadroGelen 65, PlanGeldi 16, ikon-arama, contract override anlık reactive)
4. SP definition comment-parse (cache'lenebilir, opsiyonel) — mevcut SP'lerde `RS1. Plan-Fiili kişi detay` gibi yorum varsa server-side default isim olarak kullan
5. F-9 — `Önizle` iframe (gerçek dashboard render preview)
6. Param-bar editable — Tarih chip → date picker, Çalıştır → fetchSpPreview yeni param ile
7. builder.js modülarize (440+ satır → state/grid/drawer/sp ayrı dosyalar)
8. **SP fix — PlanBeklenenAnlik + PlanPdksYok alanları** (29 Nisan 2026 kullanıcı tespiti):
   - Anomali: `sp_PdksPano` RS:3 (KPI) `PlanGelmedi=0` rapor ediyor ama gerçekte 141 - 16 (PlanGeldi) - 2 (PlanBekliyor) = 123 kişi planlı + gelmemiş. Bu kişiler `f.TC IS NULL` (PDKS'te aktif olmayan) durumda → SP `PlanGelmedi` filter `f.TC IS NOT NULL` istiyor → bypass ediliyor.
   - RS:2 (Şube Özet)'te `PdksYok` kolonu var ama RS:3 (KPI tek satır)'a yansımıyor.
   - Plus saat-anlık metric eksik: `PlanToplam` günün toplam planı (saat-bağımsız), saat 23:00'da "şu ana kadar gelmesi gereken" hesabı yok.
   - Eklenecek: `@PlanPdksYok` (PdksYok toplamı) + `@PlanBeklenenAnlik` (PlanBas <= @SimdiTime olan satırlar) → RS:3'e iki yeni kolon.
   - **Risk**: prod SP, V1 dashboard render eden tüm widget'lar etkilenebilir; fix öncesi V1 audit + smoke test, ALLOW_WRITE BKM SQL erişimi gerek.
9. **Calculated field desteği (V2 builder)** — kullanıcı widget'ta `PlanToplam - PlanBekliyor` gibi formül girip "şu ana kadar beklenenler" hesabını yapabilsin. AST parser zorunlu (eval yasak — security-principles.md). Whitelist: `+ - * /`, SUM, AVG, ROUND, IF, COALESCE. Plan 02 F-8 kapsamında zaten vardı, V2'ye uyarla.

**Karar**: Heuristic isimlendirme **kalıcı olarak çıkarıldı** (29 Nisan). Sebep: kullanıcı "ben bambaşka dashboardlar yapabilirim, sistem nasıl anlamlandıracak" → herhangi bir SP/sektör-özel mantık yanlış sonuç verir. Default sade "Veri Seti N", admin etiketleme rapor-bazlı.
...
### F-9 — Live preview + dirty + toast + Geri Al ✅ 6 Mayıs 2026
46. [x] `ReportsController.PreviewDashboardV2` — V2 builder draft preview iframe (V2Preview.cs:257, F-7/F-8 sırasında yapıldı)
47. [x] `ReportsController.DashboardValidate` — JSON yanıt (success/errors/warnings) — V2Preview.cs:396
48. [x] `builder-v2/builder-preview.js` Tam Önizle butonu → POST → iframe.srcdoc (F-7 sırasında yapıldı)
49. [~] Throttle 300ms — **kapsam dışı bırakıldı** (manuel "Tam Önizle" butonu yeterli, otomatik throttle ileride opsiyonel; 6 Mayıs 2026 kullanıcı kararı)
50. [x] Dirty state + beforeunload — Plan 04 F2.1 (Alpine `$store.builder.dirty`) ile yapıldı
51. [x] Geri Al — `lastSaveSnapshot` (page-load anında `init() → captureSnapshot()`) + topbar dispatch + `restoreSnapshot()` confirm
52. [x] Validation banner — `_ReportFormBuilderFeedbackV2.cshtml` kırmızı/sarı banner (errors+warnings)
53. [x] Toast — sağ alt köşe, 3sn auto-dismiss (success/error/warn/info 4 tip)
