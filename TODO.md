...
- [ ] **M-10 Faz 6 · Legacy fallback kaldır** (BEKLEMEDE — widget migrate sonrası) — DashboardConfig.ResolveResultSet'te `comp.ResultSet` legacy index ve `rsN` regex fallback'leri kaldırılacak. ÖNCESİ: Migration 27 widget.resultSet → widget.result (contract key) mapping zorunlu, yoksa PDKS Pano + Satış Pano render kırılır (15+16 widget hala legacy binding). Tier 3 ayrı oturum, ~1.5h.
