...
**EN ÜST ÖNCELİK (2026-05-08 sonrası)** — sıra Plan 14 + Plan 16.5 + IK quick wins birleşik:
...
- [x] **Plan 14 Faz B keşif** ✅ 2026-05-08 — Zirve `vw_PersonelDepartman` canonical, 3 firma UNION (BKM/BURSA_KÜLTÜR_MERKEZİ/ASİYE), 272 aktif, 4 seviye hiyerarşi. Memory: `project_zirve_personel_discovery.md`. Implement durmuş (Plan 18A ile birleşik).
- [ ] **Plan 18A · IK Quick Reports + Dashboard** ⭐ HIZLI KAZANIM (~4-6h, read-only, sync gerek değil) — view'ı sarmalayan 5 SP + ReportCatalog seed + İK Pano dashboard + FilterDefinition (sube/IK) aktive. Plan 18B sync'i beklemez, somut yönetim değeri.
- [x] **Plan 16.5 Faz A+B** ✅ tamamlandı (2026-05-08) — Domain primitives (BaseEntity / Markers / ServiceResult), Workflow (IWorkflow / StatusTransition / ApprovalRequest+Step + ApprovalService), Lookup (DictionaryType+Value / LookupService), DataScope (IUserDataScope / DataScopeRegistry). Migration 33+34. Test: 8 ServiceResult + 8 Approval + 12 DataScope + 15 Lookup. Faz A6'daki Attachment/ReadLog spekülatif (Plan 17 Faz I / Plan 19 ihtiyacı doğunca yazılır — YAGNI).
- [x] **Plan 14 Faz C** ✅ fiilen kapalı (2026-05-08) — C1 (`SpInjectionScope`, `ReportAccessScope`, DI, 12 test) commit `4e52a05`. C2/C3 (UserDataFilterInjector + ReportsController refactor) ayrı oturum bekliyor (riski yüksek, ROI düşük). C4-C6 (raporGrubu backfill/UI/default) raporGrubu hâlâ inactive olduğundan gereksiz.
...
- [ ] **Plan 14 · Filter Production-Readiness** (`plans/14-filter-production-readiness.md`) — Faz A ✅ tamamlandı (UserDataFilter diff audit + ortak form helper + ModelState). Faz B (IK sube OptionsQuery + migration 33) bekliyor. Faz C (Plan 16.5 Faz B sonrası IUserDataScope ile reportAccess deny-by-default). Faz D Plan 18 sonrasına ertelendi (bulk assign UI 2-3 user için overkill).
- [ ] **Plan 16 · vNext modül roadmap** (`plans/16-vnext-module-roadmap.md`) — 9 modül (17→26) port stratejisi. D:/Dev derin keşif tamam (YonetIQ AI Core + DikkatIQ + Operax + Tower + BkmArgus + tamim + hr*). Onay bekliyor.
- [ ] **Plan 16.5 · Mosaik.Core shared kit** (`plans/16.5-shared-kit.md`) — domain primitives + workflow + lookup + dataScope + AI Core. ~18h, 4 faz. Plan 17 öncesi.
- [ ] **Plan (eski 14) · Install/Deploy script** — master kurulum (DB CREATE + migration + seed + brand + admin user prompt). Plan 11 öğretisi: rapor metadata dump (ReportCatalog SQL backup) install kapsamına dahil olmalı. Numara çakışması var, yeni Plan 14 = Filter Production-Readiness; install script ileride yeni numara alacak.
