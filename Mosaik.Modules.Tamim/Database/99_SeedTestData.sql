-- Tamim Modülü Test Seed Verisi (DEV ONLY)
...
--
-- Tamim = bağlayıcı, kalıcı, "haberim yoktu" diyemeyecek konular:
...
--
-- İdempotent: BlokNo + TamimNo UNIQUE.

SET NOCOUNT ON;
GO

-- Önce eski (yanlış) seed'i temizle (sadece DEV — TamimId silinince blok orphan kalır,
-- bunları tamamen yenisiyle değiştir)
DELETE FROM dbo.GunlukBlok
WHERE BlokNo LIKE 'BLK-%'
  AND BlokNo IN (
    SELECT BlokNo FROM dbo.GunlukBlok
    WHERE Konu IN (
...
DELETE FROM dbo.Tamim
WHERE TamimNo LIKE 'TAM-%'
  AND NOT EXISTS (SELECT 1 FROM dbo.GunlukBlok b WHERE b.TamimId = dbo.Tamim.Id);
...
PRINT 'Tamim 1 (önceki gün) + 3 baglayici blok eklendi.';
GO
...
PRINT 'Tamim 2 (dun) + 4 baglayici blok eklendi.';
GO

------------------------------------------------------------
-- BUGÜN BEKLEYEN BLOKLAR (TamimId NULL — 17:00 cron test için)
------------------------------------------------------------
DECLARE @Bugun DATE = CAST(GETUTCDATE() AS DATE);
DECLARE @AdminId INT = (SELECT TOP 1 UserId FROM dbo.Users WHERE Username = N'admin');
DECLARE @IkId INT = (SELECT TOP 1 UserId FROM dbo.Users WHERE Username = N'ik');
...
VALUES
    (N'BLK-' + FORMAT(@Bugun, 'yyyyMMdd') + N'-001', @AdminId, N'Bilgi İşlem',
     N'Mosaik Portal — Tamim Modülü Devreye Alındı',
     N'Bugün itibariyle Mosaik portala Tamim & Sirküler modülü eklenmiştir. Tüm bağlayıcı duyurular bu modülde yayınlanacaktır.' + CHAR(13) + CHAR(10) + CHAR(13) + CHAR(10) +
     ...
     N'İlk giriş için sol menüden "Tamim & Sirküler" linkine tıklayın.',
...
------------------------------------------------------------
-- AuditLog "tamim_okundu" — okuma raporu test verisi
------------------------------------------------------------
DECLARE @Tamim1Id INT = (SELECT TOP 1 Id FROM dbo.Tamim ORDER BY TamimTarihi);
DECLARE @Tamim2Id INT = (SELECT TOP 1 Id FROM dbo.Tamim ORDER BY TamimTarihi DESC);
...
PRINT 'admin: 2 tamim okudu, ik: 1 tamim okudu (Tamim 1 OKUMADI — okuma raporu test verisi).';
GO
...
SELECT
    ...
    (SELECT COUNT(*) FROM dbo.GunlukBlok WHERE IsActive=1) AS Blok,
...
PRINT 'Tamim test seed tamamlandi (revize 2 — baglayici icerik).';
